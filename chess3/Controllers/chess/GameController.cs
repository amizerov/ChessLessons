...
namespace chess5.Controllers.chess
{
    public class GameController : Controller
    {
        ...
        {
            var User_ID = User.Identity.GetUserId();
            DataTable dtOld = G.db_select(@"
                select top 1 ID from Game where '{1}' in ( White_ID, Black_ID ) and IsActive = 1 order by dtc desc 
            ", User_ID);

            int Old_Game_ID = G._I(dtOld);

            if (Old_Game_ID > 0)
            {
                return Json(Old_Game_ID, JsonRequestBehavior.AllowGet);
            }
            else
            {
                ...
                if (IsNew == 1)
                {
                    ...
                    //object res = new { Game_ID = Game_ID };
