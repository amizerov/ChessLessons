...
;(function() {
  ...
  var CSS = {
    ...
    black: 'cb_black',
    board: 'cb_board',
    chessboard: 'cb_chessboard',
    highlight1: 'cb_highlight1',
    highlight2: 'cb_highlight2',
    notation: 'cb_notation',
    numeric: 'cb_numeric',
    piece: 'cb_piece',
    row: 'cb_row',
    sparePieces: 'cb_spare-pieces',
    sparePiecesBottom: 'cb_spare-pieces-bottom',
    sparePiecesTop: 'cb_spare-pieces-top',
    square: 'cb_square',
    white: 'cb_white'
  };
  //------------------------------------------------------------------------------
  ...
  function getPieseTheme() {
      var piece_theme = 'wikipedia'
      
      if (
          localStorage['cb_piece'] 
          && localStorage['cb_piece'].indexOf('cb_piece--') === 0
          && localStorage['cb_piece'].split('--')[1].length > 0
      )
          piece_theme = localStorage['cb_piece'].split('--')[1];
  ...
  function expandConfig() {
    ...
    //if (cfg.hasOwnProperty('pieceTheme') !== true ||(typeof cfg.pieceTheme !== 'string' &&typeof cfg.pieceTheme !== 'function'))
        //cfg.pieceTheme = './board/pieces/tour_new/{piece}.png';
    cfg.pieceTheme = './board/pieces/'+ getPieseTheme() +'/{piece}.png';
    // animation speeds
  ...
  */
  function buildBoard(orientation) {
    ...
    for (var i = 0; i < 8; i++) {
      html += '<div class="' + CSS.row + '"><div class="cb_row_wrap">';
      for (var j = 0; j < 8; j++) {
  ...
  function buildPiece(piece, hidden, id, wh) {
    if(wh === undefined)
      wh = false;
    var html = '<img src="' + buildPieceImgSrc(piece) + '" ';
    if (id && typeof id === 'string')
      html += 'id="' + id + '" ';
    html += 'alt="" class="' + CSS.piece +'" data-piece="' + piece + '" style="';
    if(wh === true)
  ...
  function animateSquareToSquare(src, dest, piece, completeFn) {
    ...
    $('body').append(buildPiece(piece, true, animatedPieceId, true));
  ...
  function isXYOnSquare(x, y) {
    ...
    return 'offboard';
      ...
      function captureSquareOffsets2(square, offset) {
          console.log(square, offset);
          SQUARE_ELS_OFFSETS = {};
          for (var i in SQUARE_ELS_IDS) {
              if (SQUARE_ELS_IDS.hasOwnProperty(i) !== true) continue;
              SQUARE_ELS_OFFSETS[i] = i === square ? offset : $('#' + SQUARE_ELS_IDS[i]).offset();
          }
      }
  ...
  function beginDraggingPiece(source, piece, x, y) {
    ...
    if (source === 'spare') DRAGGED_PIECE_LOCATION = 'offboard';
  ...
  */
  ...
  widget.resize = function() {
    ...
    //boardEl.css('height', boardEl.width());
    // set drag piece size
    draggedPieceEl.css({
      height: SQUARE_SIZE,
      width: SQUARE_SIZE
  ...
  function touchstartSquare(e) {
    ...
    if (validSquare(square) !== true || CURRENT_POSITION.hasOwnProperty(square) !== true)
      return;
    e = e.originalEvent;
    beginDraggingPiece2(square, CURRENT_POSITION[square],e.changedTouches[0].pageX, e.changedTouches[0].pageY);
  }
  ...
  function touchmoveWindow(e) {
    ...
    // prevent screen from scrolling
      //e.preventDefault();
      updateDraggedPiece2(DRAGGED_PIECE_LOCATION, CURRENT_POSITION[DRAGGED_PIECE_LOCATION],e.originalEvent.changedTouches[0].pageX,e.originalEvent.changedTouches[0].pageY);
  }
  function mouseupWindow(e) {
    ...
    stopDraggedPiece(location);
      ...
      function touchendWindow(e) {
          ...
          updateDraggedPiece2_first = true;
    // do nothing if we are not dragging a piece
    if (DRAGGING_A_PIECE !== true) return;
    // get the location
  ...
  // Initialization
  //------------------------------------------------------------------------------
      function beginDraggingPiece2(source, piece, x, y) {
          console.log(source, piece, x, y);
    ...
    if (source === 'spare') DRAGGED_PIECE_LOCATION = 'offboard';
      ...
      var updateDraggedPiece2_first = true;

      function updateDraggedPiece2(source, piece, x, y) {
          console.log(source, piece, x, y);
          if (updateDraggedPiece2_first) {
              ...
              if (source !== 'spare')
                  $('#' + SQUARE_ELS_IDS[source]).addClass(CSS.highlight1).find('.' + CSS.piece).css('display', 'none');
              updateDraggedPiece2_first = false;
          } else {
              // put the dragged piece over the mouse cursor
              draggedPieceEl.css({
                  left: x - SQUARE_SIZE / 2,
  ...
  function addEvents() {
      ...
      if (isTouchDevice() === true) {
          $('body').on('mousedown mousemove touchstart touchmove', '.' + CSS.piece, stopDefault);


          boardEl.on('touchstart', '.' + CSS.square, touchstartSquare);
          containerEl.on('touchstart', '.' + CSS.sparePieces + ' .' + CSS.piece, touchstartSparePiece);

          boardEl.on('click', '.' + CSS.square, function (e) {
              //console.log($(this).children('.cb_piece'));
              var location = isXYOnSquare(e.pageX, e.pageY);
              stopDraggedPiece(location);
              mouseenterSquare(e);
              //mouseleaveSquare(e);
              //$('.' + CSS.square).removeClass('cb_piece--active');
          });
          boardEl.on('touchstart', '.' + CSS.piece, function (e) {
              var square = $(this).parent().attr('data-square');
              if (validSquare(square) !== true || CURRENT_POSITION.hasOwnProperty(square) !== true)
                  return;
              $('.' + CSS.square).removeClass('cb_piece--active');
              $(this).parent().addClass('cb_piece--active');
          ...
          $('body').on('mousedown', '.' + CSS.piece, function(e){
            var square = $(this).parent().attr('data-square');
            if (validSquare(square) !== true || CURRENT_POSITION.hasOwnProperty(square) !== true)
                return;
            $('.' + CSS.square).removeClass('cb_piece--active');
