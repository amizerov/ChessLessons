...
$('#offer_draw__content').html('<div class="modal__form__title"><h3>Соперник согласился</h3></div><br><button class="btn btn-primary" onclick="clear_result_offer_draw();">Ok</button>');
};
function reject_offer_draw() {
$('#offer_draw__content').html('<div class="modal__form__title"><h3>Соперник отказался</h3></div><br><button class="btn btn-primary" onclick="modal_close(\'confirm_offer_draw\');">Ok</button>');
};
...
show_modal_new_game();
setTimeout(function() {
    $('#offer_draw__content').show();
    $('#offer_draw__hide_request').html('');
}, 500);
};
...
modal_close('request_draw');
GameStatus = "RejectDraw";
SetMove();
};
function show_modal_new_game(coin, coinD, ratg, ratgD) {
if(coin === undefined)  coin = false;
if(coinD === undefined) coinD = false;
if(ratg === undefined)  ratg = false;
if(ratgD === undefined) ratgD = false;

var text ='', stat = '', func = '';

if(gameOverResult.r3 === 'checkmate') text += 'Мат! ';
if(gameOverResult.r3 === 'stalemate') text += 'Пат! ';

if(gameOverResult.r1 === 'win') {
    text += 'Вы выиграли!';func = '';
    if(gameOverResult.r2 === 'surrender')  text += ' Соперник сдался';
    else if(gameOverResult.r2 === 'close') text += ' Соперник покинул игру';
    else if(gameOverResult.r2 === 'time')  text += ' У соперника закончилось время';
} else if (gameOverResult.r1 === 'lose') {
    text += 'Вы проиграли!';func = '';
    if(gameOverResult.r2 === 'time') text += ' У вас закочилось время';
...
if(ratg && ratgD) {
    if(ratgD.toString().indexOf('-') === -1) ratgD = '+'+ratgD;
    stat += '<p>Ваш рейтинг: '+ratg+' ( '+ratgD+' )</p>';
}

if(coin && coinD) {
    if(coinD.toString().indexOf('-') === -1) coinD = '+'+coinD;
