function engineGame(options) {
    ...
    function prepareMove() {
        ...
        var history = game.history({verbose: true});
        if(history.length > 0) {
            var flag = history[history.length - 1].flags;
            if(flag === 'k' || flag === 'q')
        ...
        if (/*!game.game_over()*/!game_over) {
            ...
            if (game.game_over()) {
                if (game.in_checkmate()) {
                    console.log('checkmated', game.turn());
                    $("#modal_gameover_wrapper").find(".form-wrapper").find("." + turn).show();
                    $("#modal_gameover_wrapper").find(".form-wrapper").find("." + (turn === 'white' ? 'black' : 'white')).hide();
                    modal_open('gameover');
                } else if (game.in_stalemate()) {
                    console.log('stalemate', game.turn());
                    $("#modal_gameover_wrapper").find(".form-wrapper").find("." + turn).show();
                    $("#modal_gameover_wrapper").find(".form-wrapper").find("." + (turn === 'white' ? 'black' : 'white')).hide();
                    modal_open('gameover');
            }
