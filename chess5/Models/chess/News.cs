...
namespace chess5.Models
{
    ...
    public class CNewsOfChessItem
    {
        ...
        public int Prev_ID { 
            get 
            {
                string[] ar = Date.Split('.'); if (ar.Length != 3) return ID;
                string d = ar[2] + "-" + ar[1] + "-" + ar[0];
                DataTable dt = G.db_select($"select top 1 ID from News where datediff(dd, dtc, '{d}') >= 0 and ID < {ID} order by dtc desc");
                int pid = G._I(dt);
                if (pid == 0) pid = ID;
                return pid;
            } 
